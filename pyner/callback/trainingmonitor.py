...
class TrainingMonitor():
    ...
    def step(self,logs={}):
        ...
        if len(self.H["loss"]) > 1:
            ...
            plt.figure()
            plt.plot(N, self.H["train_token_acc"], label="train_token_acc")
            plt.plot(N, self.H["val_token_acc"], label="val_token_acc")
            # plt.plot(N, self.H["rank"], label="train_rank5")
            # plt.plot(N, self.H["val_rank"], label="val_rank5")
            plt.legend()
            plt.xlabel("Epoch #")
            plt.ylabel("Accuracy")
            plt.title("Training Accuracy [Epoch {}]".format(len(self.H["train_token_acc"])))
            plt.savefig(self.acc_path)
            plt.close()

            plt.figure()
            plt.plot(N, self.H["train_f1"], label="train_f1")
            plt.plot(N, self.H["val_f1"], label="val_f1")
            plt.legend()
            plt.xlabel("Epoch #")
            plt.ylabel("F1 Score")
            plt.title("Training F1 Score [Epoch {}]".format(len(self.H["train_f1"])))
